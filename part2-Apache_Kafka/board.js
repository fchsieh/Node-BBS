...
function argumentParser(socket, cmd, data, position, usage) {
  ...
  if (
    ...
    (parsedCommand.length > 1 && parsedCommand[0] === "")
  ) {
    socket.write(usage);
    return;
  }
...
module.exports = {
      ...
      Board.find({}, (err, boards) => {
        if (!err) {
          for (let b of boards.sort((x, y) => {
            ...
            socket.write(
              b["BoardIndex"] + "\t" + b["BoardName"] + "\t" + b["Mod"] + "\n"
            );
          }
          socket.write("% ");
          return;
    ...
    Board.countDocuments({ BoardName: recv[1] }, (err, count) => {
      ...
      if (count === 0) {
        ...
        if (recv.length === 2) {
          ...
          let boardName = recv[1];
          Post.find(
            { Title: { $regex: key }, Board: boardName },
            (err, posts) => {
              if (!err) {
                for (let b of posts.sort((x, y) => {
                  return x.PostIndex > y.PostIndex ? 1 : -1;
    ...
    Post.countDocuments({ PostIndex: postid }, (err, count) => {
      ...
      if (count > 0) {
        Post.findOne({ PostIndex: postid }, (err, post) => {
          ...
          let month =
            (post["CreatedAt"].getMonth() < 9 ? "0" : "") +
            (post["CreatedAt"].getMonth() + 1);
          let day =
            (post["CreatedAt"].getDate() < 10 ? "0" : "") +
  ...
  comment: function (socket, recv, data) {
    if (!socket.Session.login) {
      ...
      // get comment
      let commentContent = String(data)
        .substring(String(data).indexOf(postid) + postid.length)
        .trim();
