...
stdin.addListener("data", (input) => {
  ...
  if (send.length > 0 && send[0] === "exit") {
    ...
    if (loggedin) {
      ...
      if (
        send.length !== 5 ||
        !(usageArr.indexOf(send[1]) > -1) ||
        send[3] !== "--keyword"
      ) {
        process.stdout.write(
          "Usage: subscribe --board/--author <board-name>/<author-name> --keyword <keyword>\n% "
        );
      } else {
    ...
    if (send.length !== 3 || !(usageArr.indexOf(send[1]) > -1)) {
      process.stdout.write(
        "Usage: unsubscribe --board/--author <board-name>/<author-name>\n% "
      );
    } else {
      ...
      if (loggedin) {
        if (subscription[unsubTopic] == null) {
          ...
          consumer.removeTopics([unsubTopic], (err, rmv) => {
            if (err) throw err;
          });
          subscription["SubbedTopics"].splice(
            subscription["SubbedTopics"].indexOf(unsubTopic),
            1
          );
...
function addTopic(newTopic) {
  consumer.addTopics([newTopic], (err, added) => {
    if (err) {
      ...
      kClient.createTopics(
        [{ topic: newTopic, partitions: 1, replicationFactor: 1 }],
        (err, res) => {
          if (err) throw err;
